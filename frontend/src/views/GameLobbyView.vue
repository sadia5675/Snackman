        ...
        <li v-for="player in players" :key="player.name"
          ...
          <div class="flex items-center space-x-2">
            <select v-model="player.playerrole"
                    @change="setPlayerRole(player.name, player.playerrole.toString())"
...
function setPlayerRole(playerName: string, role: string) {
  ...
  if (player) {
    player.playerrole = role == "SnackMan" ? PlayerRole.SnackMan : PlayerRole.Ghost; // Rolle des Spielers setzen
